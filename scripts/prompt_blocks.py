...
IMAGE_RULES = (
    ...
    "clip.\n"
    "3. Keep the channel identity fixed: the same warm-brown capybara, cream headphones, "
    "hand-painted storybook rendering, calm eye-level locked camera. Eyes match the activity: "
    "open or half-open whenever the capybara is using its paws (writing, pouring, turning a page), "
    "fully closed only in a resting or dozing scene. Closed eyes during a manual action read as a "
    "mistake, and they also remove the blink — the cheapest and most alive micro-motion available.\n"
    ...
    "6. GEOMETRIC ISOLATION — any motion that will be added later in post (rain, snow, mist, falling "
    "petals) must fall inside ONE region that a single polygon can enclose: one window, one doorway, "
    "one clear patch of sky. Never design a room that is glass on every side, an open balcony, or any "
    "layout where indoors and outdoors interleave across the frame. A flat PNG carries no depth, so "
    "'rain behind the glass' and 'rain inside the room' are literally the same pixels and no "
    "parameter can separate them. Test: can you draw one quadrilateral around everything that counts "
    "as outside? If not, change the setting.\n"
    "7. If the illustration already paints its own weather, that painted weather is the final weather "
    "— no second synthetic layer will be composited on top, because a generated rain layer understands "
    "glass and a drawn line does not. In that case post-production animates water only: puddle ripples, "
    "wave motion, reflections.\n"
    "8. TONAL SEPARATION — the companion animal and every key prop must differ from the character and "
    "from the surrounding palette in VALUE, not only in hue, so they stay legible at thumbnail size and "
    "in shadow. A warm-orange companion tucked into a warm-orange scene disappears; give it a light "
    "chest, a pale cushion or a rim of light to read against the dark.\n"
    "9. Output one single clean image at the stated aspect ratio and the highest resolution available."
)
...
VIDEO_RULES = (
    ...
    "6. The last frame must land back on the opening pose AND the same phase of environmental motion — "
    "not merely near it. A clip whose first and last frames differ cannot be looped and can only be hard "
    "cut, which is what destroyed the 2026-07-24 batch. Do not force an abrupt rewind; arrive there "
    "smoothly. Every clip is checked before it is kept, and one that fails is regenerated, not salvaged.\n"
    "7. The 16:9 frame is the deliverable and composition is judged on it alone. The 9:16 crop is a "
    "bonus: keep the character, the primary action and the main moving prop inside the corridor when it "
    "costs the wide composition nothing, and skip the vertical cut entirely on days where it would come "
    "out empty.\n"
    ...
    "wording.\n"
    "9. Post: remove the Veo watermark by CROPPING, never with delogo — delogo leaves visible smear "
    "streaks that look worse than the watermark it removes. Use "
    "crop=1824:1026:0:0,scale=1920:1080:flags=lanczos, which drops about 5% of the right and bottom "
    "edge and leaves the composition intact. Then build the seamless loop with an xfade of about 1.2 s "
    "before looping the clip out to album length. Encode with an explicit bitrate ceiling "
    "(-crf 23 -maxrate 9M -bufsize 18M): heavy rain or wave detail is extremely expensive to compress, "
    "and an uncapped CRF pass on this material produces a file LARGER than the master."
)
